        ...
        <nav className="flex justify-between">
          ...
          <div className={`mobile-design flex flex-col md:flex-row items-center justify-center md:justify-between gap-6 md:gap-0 md:w-2/3 -translate-y-full md:translate-y-0 absolute md:static w-full h-screen md:h-fit left-0 top-0 md:bg-inherit transition-transform ease-in-out duration-300 ${navOpen ? "translate-y-0" : ""} ${theme === "dark" ? "bg-black text-white" : "bg-white text-black"}`}>
            ...
            <ul className="nav-list flex flex-col md:flex-row items-center justify-center gap-6 w-1/2">
              <li className="capitalize active"><a href="#project">project</a></li>
            ...
            <div className="theme flex flex-col md:flex-row items-center gap-4">
              <a className={`bg-[#00CC75] py-[8px] px-[16px] cursor-pointer hover:shadow-[0_0_26px_3px] shadow-[#00CC75]/40 hover:-translate-y-0.5 transition-all ease-in-out duration-100 active hire-me ${theme === "dark" ? "text-black" : "text-white"}`} href="files/OLANREWAJU FEMI AKINWALIRE(Frontend Developer).pdf" download>Download CV</a>
              <button className={`theme-button rounded-full border border-gray-900 flex justify-center items-center p-2 h-[36px] w-[36px] md:h-[40px] md:w-[40px] hover:border-[#00CC75] hover:rotate-180 transition-transform ease-in-out duration-300 cursor-pointer`} onClick={handleToggle}>{theme === "dark" ? <Sun size={36}/> : <Moon size={36}/>}</button>
            </div>
          </div>
          <div className="hamburger flex items-center md:hidden text-white">
            <button onClick={()=>setOpenNav(true)} className="cursor-pointer">
