  ...
  return (
    <section id="work" className="md:py-16 pb-15">
      <div className="work container mx-auto px-6">
        <h2 className="text-[60px] pb-10 tracking-tight font-extralight">Selected Work</h2>
        <div className="projects-container grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project)=>{
            return (
              <motion.div className="project-cont relative p-6 border-[1.5px] border-[#2a2a2a] cursor-pointer hover:border-[#00CC75] transition-border ease-in-out duration-1000 group" key={project.id}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true }}
              variants={{
                ...
                <div className="project-content relative">
                  <ul className="stacks-container flex gap-1 mb-4">
                    {project.stacks.slice(0,3).map((stack, index)=>(
                      <li key={stack} className="tag uppercase">{index + 2 > project.stacks.length - 1 && <span className="w-1 h-1 inline-flex justify-center rounded-full bg-[#00CC75]"></span>} {stack}</li>
                    ))}
                  </ul>
